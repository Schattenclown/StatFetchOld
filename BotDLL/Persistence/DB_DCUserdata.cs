...
public class DB_DCUserdata
{
   ...
   {
      var sqlCommand = "SELECT * FROM DCUserdata";
   ...
   {
      var sqlCommand = "INSERT INTO DCUserdata (ServerInfoId, AuthorId, ChannelId, Abo, IsMinimalAbo) " + $"VALUES ({dC_UserdataObj.ServerInfoId}, {dC_UserdataObj.AuthorId}, {dC_UserdataObj.ChannelId}, {dC_UserdataObj.Abo}, {dC_UserdataObj.IsMinimalAbo})";
      DB_Connection.ExecuteNonQuery(sqlCommand);
   }

   public static void Change(DCUserdata dC_UserdataObj)
   {
      var sqlCommand = $"UPDATE DCUserdata SET Abo={dC_UserdataObj.Abo}, IsMinimalAbo={dC_UserdataObj.IsMinimalAbo} WHERE ServerInfoId={dC_UserdataObj.ServerInfoId} AND AuthorId={dC_UserdataObj.AuthorId} AND ChannelId={dC_UserdataObj.ChannelId}";
      DB_Connection.ExecuteNonQuery(sqlCommand);
   }
...
#pragma warning restore CS8604 // Mögliches Nullverweisargument.
      ...
      var sqlCommand = $"CREATE DATABASE IF NOT EXISTS `{database}`;" + $"USE `{database}`;" + "CREATE TABLE IF NOT EXISTS `DCUserdata` (" + "`DCUserdataID` MEDIUMINT NOT NULL AUTO_INCREMENT," + "`ServerInfoId` MEDIUMINT DEFAULT NULL," + "`AuthorId` BIGINT NOT NULL," + "`ChannelId` BIGINT DEFAULT NULL," + "`Abo` TINYINT DEFAULT NULL," + "`IsMinimalAbo` TINYINT DEFAULT NULL," + "PRIMARY KEY (DCUserdataID)," + "KEY (ServerInfoId)" + ") ENGINE=InnoDB DEFAULT CHARSET=latin1;";
