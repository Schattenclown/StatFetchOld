...
#pragma warning restore CS8601 // Mögliche Nullverweiszuweisung.
      ...
      {
         var quickChartString = $"https://quickchart.io/chart/render/{token}?title={serverInfoObj.Name?.Replace(" ", "%20")}";
         ...
         foreach (var monthStatisticsItem in serverInfoObj.MonthStatisticsList)
         {
            if (ii < 30)
            {
         ...
         for (var i = 0; i < 30; i++) data2 += $"{data2List[i]},";
