...
internal class ApplicationCommandsEvents
{
   ...
   {
      Console.WriteLine($"Main/Error: {e.Exception.Message} | CN: {e.Context.CommandName} | IID: {e.Context.InteractionId}");
      return Task.CompletedTask;
   }
   ...
   public static Task Client_ApplicationCommandPermissionsUpdated(DiscordClient sender, ApplicationCommandPermissionsUpdateEventArgs e)
   {
      ...
      foreach (var perm in e.Permissions) Console.WriteLine($"Permission with type {perm.Type} got updated for {perm.Id}. Command {e.Command.Name} is {(perm.Permission ? "allowed" : "denied")}.");
      return Task.CompletedTask;
   }
}