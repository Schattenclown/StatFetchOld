...
namespace BotDLL.Model.BotCom.Discord.Interactions
{
   /// <summary>
   /// The slash commands.
   /// </summary>
   internal class Slash : ApplicationCommandsModule
   {
      ...
      public static async Task ShowServerStatusAsync(InteractionContext interactionContext, [ChoiceProvider(typeof(FourtytwoTypeChoiceProvider))][Option("Type", "Type")] string fourtytwoChoice)
      {
         ...
         IEnumerable<DiscordApplicationCommandOptionChoice>? fourtytwoTypeChoices = await fourtytwoTypeChoiceProvider.Provider();
...
#pragma warning restore CS8602 // Dereferenzierung eines möglichen Nullverweises.
         ...
         Uri? qcUri = QCUriGenerator.CreateObj(serverInfoObj).QCUri;
